...
namespace ZKAttendance.Infrastructure.Devices
{
    ...
    // =========================================================
        /// <summary>Anviz biometric device reader via proprietary TCP binary protocol.
    /// TODO: implement with the vendor SDK or HTTP API.
    /// Configuration section: Anviz in appsettings.json
    /// </summary>
    public sealed class AnvizDeviceReader : IZkDeviceReader
    {
        private readonly ILogger<AnvizDeviceReader> _logger;
        public AnvizDeviceReader(ILogger<AnvizDeviceReader> logger) => _logger = logger;

        public Task<bool> ConnectAsync(string ip, int port, int commPassword = 0)
        {
            _logger.LogWarning("AnvizDeviceReader.ConnectAsync not yet implemented for {IP}", ip);
        ...
        public Task<EnrollResult> StartRemoteEnrollAsync(string id, int finger = 0) =>
            Task.FromResult(new EnrollResult(false, "Not implemented for AnvizDeviceReader", finger));
