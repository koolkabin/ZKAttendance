...
namespace ZKAttendance.Infrastructure.Devices
{
    ...
    // =========================================================
        /// <summary>Generic HTTP-push receiver. The device posts punches to this agent; this reader returns them from an in-memory queue populated by the push controller.
    /// TODO: implement with the vendor SDK or HTTP API.
    /// Configuration section: HttpPush in appsettings.json
    /// </summary>
    public sealed class HttpPushDeviceReader : IZkDeviceReader
    {
        private readonly ILogger<HttpPushDeviceReader> _logger;
        public HttpPushDeviceReader(ILogger<HttpPushDeviceReader> logger) => _logger = logger;

        public Task<bool> ConnectAsync(string ip, int port, int commPassword = 0)
        {
            _logger.LogWarning("HttpPushDeviceReader.ConnectAsync not yet implemented for {IP}", ip);
        ...
        public Task<EnrollResult> StartRemoteEnrollAsync(string id, int finger = 0) =>
            Task.FromResult(new EnrollResult(false, "Not implemented for HttpPushDeviceReader", finger));
