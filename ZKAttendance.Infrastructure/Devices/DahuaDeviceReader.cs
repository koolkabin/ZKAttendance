...
namespace ZKAttendance.Infrastructure.Devices
{
    ...
    // =========================================================
        /// <summary>Dahua access-control panel reader via HTTP/TCP SDK.
    /// TODO: implement with the vendor SDK or HTTP API.
    /// Configuration section: Dahua in appsettings.json
    /// </summary>
    public sealed class DahuaDeviceReader : IZkDeviceReader
    {
        private readonly ILogger<DahuaDeviceReader> _logger;
        public DahuaDeviceReader(ILogger<DahuaDeviceReader> logger) => _logger = logger;

        public Task<bool> ConnectAsync(string ip, int port, int commPassword = 0)
        {
            _logger.LogWarning("DahuaDeviceReader.ConnectAsync not yet implemented for {IP}", ip);
        ...
        public Task<EnrollResult> StartRemoteEnrollAsync(string id, int finger = 0) =>
            Task.FromResult(new EnrollResult(false, "Not implemented for DahuaDeviceReader", finger));
